...
def GenDataset(img, craters, outhead, rawlen_range=[1000, 2000],
    ...
    for i in range(amt):
        ...
        if imgo is None:
            ...
            continue
        #imgo.save('input_data/'+str(i)+'.jpg')
        ...
        print(imgo_arr.shape)
        print(mask.shape)
        # matplotlib.image.imsave("valid/test1/"+str(i)+'_mask.jpg', mask)
        # img_c=cv2.imread("valid/test1/"+str(i)+'_mask.jpg')
        # img_mid=cv2.resize(img_c,(128,128))
        # img_min=cv2.resize(img_c,(64,64))
        # cv2.imwrite("valid/test1/"+str(i)+'_mid_mask.jpg',img_mid)
        # cv2.imwrite("valid/test1/"+str(i)+'_min_mask.jpg',img_min)
