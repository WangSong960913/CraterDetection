...
def template_match_t2c(target, csv_coords, minrad=minrad_, maxrad=maxrad_,
    ...
    #print((csv_coords.shape))#(82, 3)
    s_csv_coords = []
    #s_csv_coords=csv_coords
    for item in csv_coords:
        if item[2]>=5-0.5 and item[2]<=40+0.5:
            s_csv_coords.append(item)

            #s_csv_coords = np.append(s_csv_coords, item, axis=0)
    s_csv_coords_np=np.array(s_csv_coords)
    N_csv, N_detect = len(s_csv_coords_np), len(templ_coords)
    for lo, la, r in templ_coords:
        print(type(csv_coords))
        Long, Lat, Rad = s_csv_coords_np.T
        minr = np.minimum(r, Rad)
        ...
        s_csv_coords_np = s_csv_coords_np[np.where(index == False)]
        if len(csv_coords) == 0:
            break
